...
_THEME_VARIANTS=('' '-Purple' '-Pink' '-Red' '-Orange' '-Yellow' '-Green' '-Blue' '-Grey')
...
Tar_themes() {
  ...
  for scheme in "${_SCHEME_VARIANTS[@]}"; do
    tar -Jcvf ${THEME_NAME}${theme}${scheme}.tar.xz ${THEME_NAME}{'','-Purple','-Pink','-Red','-Orange','-Yellow','-Green','-Blue','-Grey'}{'','-Light','-Dark'}${scheme}
  done
}
...
cd .. && ./install.sh -d $THEME_DIR -t all && ./install.sh -d $THEME_DIR --tweaks nord -t all && ./install.sh -d $THEME_DIR --tweaks dracula -t all
