...
for theme in '' '-Purple' '-Pink' '-Red' '-Orange' '-Yellow' '-Green' '-Blue' '-Grey'; do
  for type in '' '-Nord' '-Dracula'; do
    ...
    if [[ "$type" != '' ]]; then
      ...
      sed -i "s/#009688/${theme_color_dark}/g" "theme${theme}${type}"/*.svg
      sed -i "s/#4db6ac/${theme_color_light}/g" "theme${theme}${type}"/*.svg
    elif [[ "$theme" != '' ]]; then
      rm -rf "theme${theme}"
      cp -rf "theme" "theme${theme}"
      sed -i "s/#009688/${theme_color_dark}/g" "theme${theme}"/*.svg
      sed -i "s/#4db6ac/${theme_color_light}/g" "theme${theme}"/*.svg
    fi
  done
done
