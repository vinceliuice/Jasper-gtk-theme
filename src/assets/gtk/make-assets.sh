...
for theme in '' '-Purple' '-Pink' '-Red' '-Orange' '-Yellow' '-Green' '-Blue' '-Grey'; do
  for type in '' '-Nord' '-Dracula'; do
    ...
    if [[ "$type" != '' ]]; then
      ...
      sed -i "s/#009688/${theme_color_dark}/g" "assets${theme}${type}.svg"
      sed -i "s/#4DB6AC/${theme_color_light}/g" "assets${theme}${type}.svg"
      ...
      sed -i "s/#009688/${theme_color_dark}/g" "assets${theme}.svg"
      sed -i "s/#4DB6AC/${theme_color_light}/g" "assets${theme}.svg"
    fi
  done
done
